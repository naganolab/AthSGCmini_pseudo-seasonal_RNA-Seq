...
### venn diagram of SOgene 1 vs 3 vs 7 days, Fig. S ?-----------------------------------------------
library(VennDiagram)

vennlist <- list(d1=rownames(alpha.all[alpha.all$Ath_d1 > 1,]),
                 d3=rownames(alpha.all[alpha.all$Ath_d3 > 1,]),
                 d7=rownames(alpha.all[alpha.all$Ath_d7 > 1,]))
venn.diagram(vennlist, filename="plot_Fig/SOGene_venn_d137.tif", 
...
### venn diagram of mimiced genes in 7,3,1 day conditions, Fig. 6C -----------------------------------------------
library(VennDiagram)
...
## d7 mimicked gene, Supplementary Table 3, 12
tmp <- rownames(alpha.SO[mic.gene$Ath_d7,])
fn <- "GOanalysis/GoOut_d7mic.csv"
fn.GOanalysis(tmp = tmp, fn = fn, ulg2 = ulg2)

## d3 mimicked gene, Supplementary Table 13
tmp <- rownames(alpha.SO[mic.gene$Ath_d3,])
fn <- "GOanalysis/GoOut_d3mic.csv"
fn.GOanalysis(tmp = tmp, fn = fn, ulg2 = ulg2)

## d1 mimicked gene, Supplementary Table 14
tmp <- rownames(alpha.SO[mic.gene$Ath_d1,])
fn <- "GOanalysis/GoOut_d1mic.csv"
fn.GOanalysis(tmp = tmp, fn = fn, ulg2 = ulg2)
...
xlabel <- c("total SR gene","total non-SR gene","non-SR gene(phase)")
GOlist$ylabel <- factor(GOlist$ylabel, levels = unique(GOlist$ylabel))

## draw GO plot (only biologicl process)
pdf("GOanalysis/REVIGO_Biological_0.5/SRgeneGOplot_Biological.pdf", width = 5.5, height = 6)
...
# GO plot from REVIGO file of Ath SR genes, Fig. S? -------------------------------------------------------
## read REVIGO file
ord <- c("d7","d3", "d1")

GOlist <- NULL
for(j in ord){　　　# check cluster order
  fn <- sprintf("GOanalysis/GoOut_%smic.csv", j) 
  tmp <- read.csv(fn, row.names = 1,stringsAsFactors = F) 
  fn <- sprintf("GOanalysis/REVIGO_Biological_0.5/Revigo_BP_%smic.tsv", j) 
  tmp2 <- read.delim(fn, stringsAsFactors = F)
  
  tmp2 <- tmp2[tmp2$Representative == "null",]
  tmp <- tmp[tmp2$TermID,]
  tmp$day <- rep(sprintf("%s", j), nrow(tmp))
  rownames(tmp) <- NULL
  tmp <- tmp[order(tmp$Adjusted.P.value),]
...
GOlist$day <- factor(GOlist$day, levels = c("d7", "d3", "d1"))

## draw GO plot 
library(ggplot2)

pdf("plot_Fig/Ath_SRgenes_GOplot.pdf", width = 5.5, height = 4) 
g <- ggplot(GOlist, aes(x = ylabel, y = day)) +
  geom_point(data = GOlist,
             aes(x = ylabel, y = day, size = Gene_number, colour = mlog10P), 
             alpha = .7) +
...
# GO plot from REVIGO file of Ath SO genes, Fig. S? -------------------------------------------------------
## read REVIGO file
ord <- c("d7","d3", "d1")

GOlist <- NULL
for(j in ord){　　　# check cluster order
    fn <- sprintf("GOanalysis/GoOut_AthSOgenes_%s.csv", j) 
    tmp <- read.csv(fn, row.names = 1,stringsAsFactors = F) 
    fn <- sprintf("GOanalysis/REVIGO_Biological_0.5/Revigo_BP_%s.tsv", j) 
    tmp2 <- read.delim(fn, stringsAsFactors = F)
...
pdf("Ath_SOgenes_GOplot.pdf", width = 6, height = 7.5) 
g <- ggplot(GOlist, aes(x = ylabel, y = day)) +
  geom_point(data = GOlist,
             aes(x = ylabel, y = day, size = Gene_number, colour = mlog10P), 
             alpha = .7) +
